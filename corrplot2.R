          ...
          uppCI.mat = NULL, addCoef.cex = 0.8, addCoef.font = 2, addCoef.labels="all", ...) 
{
  ...
  if (plotCI != "n") {
    ...
    if (plotCI == "square") {
      symbols(pos.uppNew[, 1], pos.uppNew[, 2], add = TRUE, 
              inches = FALSE, squares = abs(bigabs)^0.5, bg = ifelse(sig > 0, col.fill, col[ceiling((bigabs + 1) * length(col)/2)]), 
    ...
    ############################## edit start #################################
    if (plotCI == "fullcircle") {
      bg <- fg <- col[ceiling((bigabs + 1) * length(col)/2)]
      symbols(pos.uppNew[, 1], pos.uppNew[, 2], add = TRUE, 
              inches = FALSE, circles = 0.95 * abs(bigabs)^0.5/2, 
              bg = bg, fg = fg)  # add outer circle, high CI
      
      symbols(Pos, add = TRUE, inches = FALSE, bg = col.fill, 
              circles = 0.95 * abs(DAT)^0.5/2, fg = col.border)  # add middle circle, regular corr. coefficient
      
      bg = col[ceiling((smallabs + 1) * length(col)/2)]
      symbols(pos.lowNew[, 1], pos.lowNew[, 2], add = TRUE, 
              inches = FALSE, circles = 0.95 * abs(smallabs)^0.5/2, 
              bg = bg, fg = ifelse(sig > 0, col.fill, col[ceiling((smallabs + 1) * length(col)/2)]))  # add small circle, low CI
      
      symbols(Pos, add = TRUE, inches = FALSE, bg = rgb(0,0,0,alpha=0), 
              circles = 0.95 * abs(DAT)^0.5/2, fg = col.border)  # add empty circle with border, to make sure outline shows
    }
    if (plotCI == "fullsquare") {      
      bg <- fg <- col[ceiling((bigabs + 1) * length(col)/2)]
      symbols(pos.uppNew[, 1], pos.uppNew[, 2], add = TRUE, 
              inches = FALSE, squares = abs(bigabs)^0.5, bg = bg, 
              fg = fg) # add outer square, high CI
      
      symbols(Pos, add = TRUE, inches = FALSE, squares = abs(DAT)^0.5, 
              bg = col.fill, fg = col.border)   # add middle square, for coefficient
      
      bg = col[ceiling((smallabs + 1) * length(col)/2)]
      symbols(pos.lowNew[, 1], pos.lowNew[, 2], add = TRUE, 
              inches = FALSE, squares = abs(smallabs)^0.5, 
              bg = bg, fg = ifelse(sig > 0, col.fill, col[ceiling((smallabs + 1) * length(col)/2)]))  # add small square, low CI
      
      symbols(Pos, add = TRUE, inches = FALSE, squares = abs(DAT)^0.5, 
              bg = rgb(0,0,0,alpha=0), fg = col.border)  # add empty square with border, to make sure outline shows
...
colorlegend <- function (colbar, labels, at = NULL, xlim = c(0, 1), ylim = c(0, 
                                                              1), vertical = TRUE, ratio.colbar = 0.4, lim.segment = NULL, 
    ...
    if (addlabels) {
      ...
      if (align == "c") 
        text((pos.xlabel + xlim[2])/2, y = at, labels = labels, 
             ...)
      if (align == "r") 
        text(x = xlim[2], y = at, labels = labels, pos = 2, 
             ...)
    }
  }
  if (!vertical) {
